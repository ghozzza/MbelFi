...
export const useSupplyLiquidity = (chainId: number, decimals: number, onSuccess: () => void) => {
  ...
  useEffect(() => {
    if (isSuccess && onSuccess) {
      ...
      toast.success("Liquidity supplied successfully!", {
        description: "Your liquidity has been added to the lending pool.",
        duration: 5000,
        style: {
  ...
  const handleSupplyLiquidity = async (lendingPoolAddress: HexAddress) => {
    if (!address) {
