...
export const useWithdrawLiquidity = (chainId: number, decimals: number, onSuccess: () => void) => {
  ...
  useEffect(() => {
    if (isSuccess && onSuccess) {
      ...
      toast.success("Liquidity withdrawn successfully!", {
        description: "Your liquidity has been withdrawn from the lending pool.",
        duration: 5000,
        style: {
  ...
  useEffect(() => {
    if (isError && confirmError) {
      toast.error("Withdraw failed to confirm", {
  ...
  useEffect(() => {
    if (writeError) {
      toast.error("Withdraw failed to submit", {
